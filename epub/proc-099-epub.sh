...
OUTPUT_EPUB="${OUTPUT_DIR}/out-099.epub"
...
TEMP_DIR="epub-temp"
unzip -q $OUTPUT_EPUB -d $TEMP_DIR
for file in $(find "$TEMP_DIR/EPUB/text" -name '*.xhtml'); do
  echo $file
  perl -i -pe 's/<a\s+[^>]*class="footnote-back".*>\K\. +//gi' "$file"
done
rm $OUTPUT_EPUB 
zip -qr $OUTPUT_EPUB $TEMP_DIR/*
rm -rf $TEMP_DIR
